...
async function main() {
    try {
        ...
        const data = await collection.find({}, {projection:{ _id: 1, ExternalCause:1, name:1 }}).toArray();
        ...
        await collection.updateMany({ _id: { $in: ids } }, { $set: { ExternalCause: 'Unknown', name: 'Unknown' } })
